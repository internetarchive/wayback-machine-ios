...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:COLOR_HAS_ARCHIVE].CGColor);
...
- (void)drawTodayCircleWithContext:(CGContextRef)context inRect:(CGRect)rect
{
    ...
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:COLOR_TODAY].CGColor);
...
- (void)drawDayCircle:(SSDayNode *)day withContext:(CGContextRef)context inRect:(CGRect)rect
{
    if (day.hasEvents > 0) {
//        CGContextAddEllipseInRect(context, rect);
//        CGContextSetStrokeColorWithColor(context, [UIColor colorWithHexString:COLOR_HAS_ARCHIVE].CGColor);
...
- (void)drawTextForToday:(SSDayNode *)day withRadius:(CGFloat)radius inRect:(CGRect)rect
{
    ...
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    [paragraphStyle setAlignment:NSTextAlignmentCenter];
    NSDictionary *textFontAttributes = @{NSFontAttributeName: font, NSForegroundColorAttributeName: UIColor.whiteColor, NSParagraphStyleAttributeName: paragraphStyle};
    [dayText drawInRect:rect withAttributes:textFontAttributes];
}
