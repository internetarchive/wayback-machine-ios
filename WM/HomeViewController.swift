...
class HomeViewController: UIViewController, UITextFieldDelegate, MBProgressHUDDelegate {
    ...
    @IBAction func _onSave(_ sender: Any) {
        ...
        if (!verifyURL(url: getURL(url: self.txtURL.text!))) {
            ...
            WMAPIManager.sharedManager.checkURLBlocked(url: getURL(url: self.txtURL.text!), completion: { (isBlocked) in
                ...
                if isBlocked {
                    self.showErrorMessage(message: "That site's robots.txt policy requests we not archive it")
                    return
                }
                self.showWebPage(url: "https://web.archive.org/save" + self.getURL(url: self.txtURL.text!))
            })
    ...
    @IBAction func _onRecent(_ sender: Any) {
        ...
        if (!verifyURL(url: getURL(url: self.txtURL.text!))) {
            showErrorMessage(message: WMConstants.errors[201]!)
        } else {
            showProgress()
    ...
    func showCalendarVC(archives: [Dictionary<String, Any>]) {
        var events = [SSEvent]()
        var years = [Int]()
...
//        years = [2014, 2015, 2016, 2017];
    ...
    func hideProgress(_ result: Any) -> Void {
